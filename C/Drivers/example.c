...
int main()
{
	...
	char filename[1000];					  /* name of files */
	...
	for (func_num = 1; func_num <= 1; func_num++)
	{
		...
		if ((GKLS_info = fopen(filename, "wt")) == NULL)
			return (-1);
		...
		if (GKLS_dim == 2)
		{
			...
			if ((min_info = fopen(filename, "wt")) == NULL)
				return (-1);
		}

		// double t1[] = {-0.5, 0.5};
		// printf("%lf\n", GKLS_D_func(t1)); exit(0);
	...
	if ((fderiv = fopen(filename, "wt")) == NULL)
		return (-1);
